...
datos<-data.frame(tSNE1=df_tsne[,1],tSNE2=df_tsne[,2],diagnostico=df$exam_quantile)
head(datos)
...
tsne_df <- data.frame(
  tSNE1 = df_tsne[, 1],
  tSNE2 = df_tsne[, 2],
  diagnostico = df$exam_quantile
)
colors <- c("#E6194B", "#3CB44B","#FFA500")
...
datos1<-data.frame(Rtsne1=df_Rtsne$Y[,1],Rtsne2=df_Rtsne$Y[,2],diagnostico=df$exam_quantile)
head(datos1)
...
tsne_df3 <- data.frame(
  ...
  tSNE2 = df_tsne3[, 2],
  tSNE3 = df_tsne3[, 3],
  diagnostico = factor(df$exam_quantile)
)
...
colores = c("#CC0033", "#e319dc", "#FFA500")
names(colores) = c("low","medium","high")
...
datos2<-data.frame(Rtsne1=df_RtsneF$Y[,1],Rtsne2=df_RtsneF$Y[,2],diagnostico=df$exam_quantile)
